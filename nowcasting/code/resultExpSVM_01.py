...
def fnc_init():
  ...
  df_stat = pd.read_csv('../data/ecos01_macroQQ2002.csv',sep='\t')
  dfm =  pd.read_csv('../data/resultData_Monthly.csv',sep='\t')
  dfw =  pd.read_csv('../data/resultData_Weekly.csv',sep='\t')
...
def fnc_evaluateChrome(df_statResize,dateFrTo,int_popNum):
  ...
  clf=svm.SVR()
  arr_yVal = np.array(df_statResize[idx_y])[:-2]
  ...
  dbl_calcweight = 0.2 #1을 주면 fitting한 값만 평가하는 것임
...
def fnc_execGa():
  ...
  arr_frPoint=[6]  #11은 위기구간 <- 예측률이 떨어질때 #6은 예측률이 잘맞을때
  #lst_finRes=[]
  for p in arr_frPoint: #기간에 따른 조건 변경
    ...
    dfm = dfm.ix[period_q.values[0]:period_q.values[1]][1:]
    dfw = dfw.ix[period_q.values[0]:period_q.values[1]][1:]
